...
const PLAYER_RADIUS = window.innerWidth / 800;
...
const ENEMY_RADIUS = window.innerWidth / 800;
...
class DndInvaders{
    constructor(canvas, vertexShaderSource, fragmentShaderSource) {
        ...
        this.player = new Player(
            ...
            GraphicsGenerator.generatePlayerTextureCords(),
            0,
            [0, -38, 0],
    ...
    shoot(time){
        if(this.lastPleyerShootTime + RELOAD_TIME < time){
            ...
            this.projectiles.push(new Projectile(
                ...
                GraphicsGenerator.generateProjectileTextureCords(),
                2,
    ...
    generateEnemies(col=10, row=2, color=ENEMY_COLOR){
        var groupStep = 5;
        for(var i = 1; i <= row; i++){
            for(var j = 1; j <= col; j++){
                this.enemies.push(new Enemy(
                        ...
                        GraphicsGenerator.generateEnemyTextureCords(),
                        1,
...
class GameObject {
    constructor(hitRadius, points, textureCords, textureId, translation, type) { // Translation tutaj będzie 3 eementowyą tablicą, odpowiednio x, y,z. Kolor 4 elementową.
...
class Player extends GameObject {
    constructor(hitRadius, points, textureCords, textureId, translation, type) {
        super(hitRadius, points, textureCords, textureId, translation, type);
        this.pressedKeys = {
            shoot: false,
            left: false,
            right: false,
...
class Enemy extends GameObject {
    constructor(hitRadius, points, textureCords, textureId, translation, type, speed) {
        super(hitRadius, points, textureCords, textureId, translation, type);
        this.speed = speed;
        this.vy = 0;
    }
